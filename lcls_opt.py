...
from beam_io import setquad, setinjector, get_beamsizes

show_plots_here = False


class LclsOpt():
    def __init__(self, init_scan=[-2.5, -1.8, -1.5, 0, 0.5]):  # [-5, -3, -1]
        self.energy = 0.135
        ...
        self.uncertainty_lim = 0.85
        self.pbounds = ((0.44, 0.48), (-0.01, 0.01), (-0.01, 0.01))

    def get_adapted_quad_list(self, quad_list, bs_list, err, axis, num_points=5, show_plots=False):
        """
        ...
        """
        try:
            new_quad_arr = adapt_range(quad_list, bs_list, axis=axis, w=err, num_points=num_points,
    ...
    def get_beamsizes_from_machine(self, varx, vary, varz, varscan):
        ...
        for quad in varscan:
            ...
            x_rms.append(beamsize[0])
            y_rms.append(beamsize[1])
            x_err.append(beamsize[2])
            y_err.append(beamsize[3])
        return x_rms, y_rms, x_err, y_err
    ...
    def evaluate(self, varx, vary, varz):
        ...
        new_quad_list_x = self.get_adapted_quad_list(self.varscan, x_rms, x_err, 'x', num_points=self.num_points_adapt,
                                                     show_plots=show_plots_here)
        new_quad_list_y = self.get_adapted_quad_list(self.varscan, y_rms, y_err, 'y', num_points=self.num_points_adapt,
                                                     show_plots=show_plots_here)
        ...
        print(x_rms, new_quad_list_x)
        print(y_rms, new_quad_list_y)

        ##### GET EMITTANCE ####
        norm_emit = get_normemit(self.energy, new_quad_list_x, new_quad_list_y, x_rms, y_rms, xrms_err=x_err,
                                 yrms_err=y_err,
        ...
        norm_emit_err = norm_emit_sqrt * (
                    (norm_emit[2] / norm_emit[0]) ** 2 + (norm_emit[3] / norm_emit[1]) ** 2) ** 0.5
    ...
    def run_bo_opt_w_reject(self, rnd_state=11, init_pnts=3, n_iter=120):
        ...
        x = [[0.44876230514522875, 0.007868983426579103, -0.0055396938095385465],
             [0.46396935669594885, 0.007745107629659008, -0.007883067351682538],
             [0.4589, -0.002, -0.002]]

        emits = [-0.59194594, -1.15368973, -0.6143289]
        # get init points
        for i in range(len(x)):
            ...
            print(
                f"{color[0]}iter {i} | target {-1 * target:.3f} | config {next_point['varx']:.6f} {next_point['vary']:.6f} {next_point['varz']:.6f}{color[1]}")
