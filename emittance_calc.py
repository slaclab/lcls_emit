...
m_0 = 0.000511
d = 12.327300 - 8.548049
l = 0.108
twiss0 = beamline_info['Twiss0']
energy = beamline_info['energy']

# input matlab model matrix here
mat2 = np.array([[1.1055070102091, 2.74258378560201], [-0.213608624301716, 0.374633943250189]])
...
def fit_sigma(sizes, k, axis, sizes_err=None, d=d, l=l, adapt_ranges=False, num_points=5, show_plots=False):
    ...
    if sizes_err is not None and sizes.all() > 0 and sizes_err.all() > 0:
        w = 2 * sizes_err  # sigma for poly fit
    ...
    if adapt_ranges:
        try:
            coefs, cov, k = adapt_range(k, sizes, w=w, axis=axis, fit_coefs=coefs, x_fit=xfit, energy=energy,
                                        num_points=num_points, save_plot=True, show_plots=show_plots)
            # log data
    ...
    try:
        with warnings.catch_warnings():
            ...
            # print(emit, emit_err)
            return emit, emit_err, coefs, coefs_err, k
...
def get_normemit(energy, xrange, yrange, xrms, yrms, xrms_err=None, yrms_err=None,
    ...
    emitx, emitx_err, coefsx, coefsx_err, kx_final = fit_sigma(np.array(xrms), kx, axis='x',
                                                               sizes_err=np.array(xrms_err),
                                                               adapt_ranges=adapt_ranges, num_points=num_points,
                                                               show_plots=show_plots)

    emity, emity_err, coefsy, coefsy_err, ky_final = fit_sigma(np.array(yrms), -ky, axis='y',
                                                               sizes_err=np.array(yrms_err),
                                                               adapt_ranges=adapt_ranges, num_points=num_points,
                                                               show_plots=show_plots)
...
def adapt_range(x, y, axis, w=None, fit_coefs=None, x_fit=None, energy=energy, num_points=5, save_plot=False,
    ...
    if fit_coefs is None:
        ...
        plot_fit(k, y, x_fit, yerr=None, axis=axis, save_plot=False, title_suffix="initial", show_plots=show_plots)
    ...
    if np.sum(fine_fit_sizes_err) == 0:
        ...
        w = 2 * fine_fit_sizes_err  # since we are squaring the beamsize
...
def numpy_save(norm_emitx, norm_emity, bmagx, bmagy, norm_emitx_err, norm_emity_err, bmagx_err, bmagy_err, beta_quad_x,
               alpha_quad_x, beta_quad_y, alpha_quad_y, bmag, opt_quad, timestamp=False,
               savelist=pv_savelist['scalars'], path=savepaths['emit_saves']):
    ts = isotime()
