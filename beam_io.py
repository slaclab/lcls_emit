...
def saveimage(im, ncol, nrow, impath = savepaths['images'],avg_img = False):
    ...
    if avg_img:
        ...
        np.save(impath+f'img_avg_{timestamp}.npy', im)
        np.save(impath+f'ncol_avg_{timestamp}.npy', ncol)
        np.save(impath+f'nrow_avg_{timestamp}.npy', nrow)
        
    else:
        timestamp = (datetime.datetime.now()).strftime("%Y-%m-%d_%H-%M-%S-%f")
        np.save(impath+f'img_{timestamp}.npy', im)
        np.save(impath+f'ncol_{timestamp}.npy', ncol)
        np.save(impath+f'nrow_{timestamp}.npy', nrow)
    

def savesummary(beamsizes):
...
def get_beam_image(subtract_bg = subtract_bg):
...
#             im = np.mean(np.array([ im,im_tmp ]), axis=0 )
    ...
    return beamsizes, beam_image.proc_image


def getbeamsizes_from_img(num_images = n_acquire, avg = avg_ims, subtract_bg = subtract_bg):
    ...
    for i in range(0,num_images):
        ...
        while repeat:
            xrms[i], yrms[i], xrms_err[i], yrms_err[i], xamp[i], yamp[i], im[i] = get_beam_image(subtract_bg)
    ...
    if avg == True:
        ...
        if mean_xamp[i]<amp_threshold or mean_yamp[i]<amp_threshold:
            return np.nan, np.nan, np.nan, np.nan, np.nan, np.nan
        
        
        #save_beam = list(np.array(beamsizes[0:4])*resolution/1e-6)
        saveimage(im, ncol, nrow, beamsizes,avg_image=True) # pass beamsizes in um
...
def get_beamsizes(use_profMon=False, reject_bad_beam=True):
    ...
    if reject_bad_beam:
        ...
        while xrms<=min_sigma or yrms<=min_sigma or xrms>max_sigma or yrms>max_sigma or np.isnan(beamsizes).any():
            if count > 1:
            ...
            if use_profMon:
                ...
                beamsizes = np.array(getbeamsizes_from_img())
                #print(beamsizes)
    ...
    save_config(xrms,yrms,xrms_err,yrms_err)
